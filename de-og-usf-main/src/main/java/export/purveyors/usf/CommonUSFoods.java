...
public class CommonUSFoods {
    ...
    public static String listUrl1="https://www.foodservicedirectorder.com/order/faces/oracle/webcenter/portalapp/pages/lists/myLists.jspx";
    ...
    // Download icon
    @FindBy(xpath = "//*[@id='dgfSPT:pt_pgl39']/div/span")
    WebElement lnk_accountName;

    @FindBy(xpath = "//*[@id='dgfSPT:pt_ot4']/span")
    WebElement lnk_accountNameList;
...
//		wait = new WebDriverWait(driver, 30);
        try {
            ...
            wait.until(ExpectedConditions.visibilityOf(com.lnk_accountNameList));
            String currentAccountName = com.lnk_accountNameList.getText();
            if (!currentAccountName.contains(account)) {
                chooseAccount(account);
            } else {
                System.out.println("required account already selected !");
...
//		wait = new WebDriverWait(driver, 30);
        try {
            ...
            Thread.sleep(3000);
            driver.get(listUrl);
            if(driver.getCurrentUrl().contains("myLists")) {
                System.out.println("This Correct Page !!");
            }else {
                System.out.println("Opening new Url for Correct List page");
                driver.get(listUrl1);
            }
